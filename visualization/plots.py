...
def plot_bar_chart(
    ...
    try:
        ...
        fig = px.bar(
            ...
            text_auto=True, **px_kwargs
        )
        fig.update_traces(
            texttemplate='%{y:,.0f}' if y_is_int else '%{y:,.2f}', textposition='outside',
            marker_line_width=1.5, marker_line_color=settings.COLOR_BACKGROUND_CONTENT,
