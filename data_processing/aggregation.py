...
class ClinicKPIPreparer:
    ...
    def _calculate_testing_kpis(self):
        if not all(c in self.df.columns for c in ['test_type', 'test_result', 'test_turnaround_days', 'sample_status', 'patient_id']):
            return
    ...
    def _calculate_supply_kpis(self):
        if not all(c in self.df.columns for c in ['item', 'item_stock_agg_zone', 'consumption_rate_per_day', 'encounter_date', 'zone_id']):
            return
    ...
    def _calculate_test_breakdown(self):
        if not all(c in self.df.columns for c in ['test_type', 'test_result', 'patient_id', 'sample_status', 'test_turnaround_days']):
            return
        ...
        agg_spec = {
            'pending_count_patients': pd.NamedAgg(column='is_pending', aggfunc='sum'),
            'rejected_count_patients': pd.NamedAgg(column='is_rejected', aggfunc='sum')
        }
        breakdown = tests_df.groupby('test_type').agg(**agg_spec)

        if not conclusive_df.empty:
            conclusive_agg_spec = {
                'positive_rate_perc': pd.NamedAgg(column='is_positive', aggfunc=lambda x: x.mean() * 100),
                'avg_tat_days': pd.NamedAgg(column='test_turnaround_days', aggfunc='mean'),
                'perc_met_tat_target': pd.NamedAgg(column='tat_met', aggfunc=lambda x: x.mean() * 100)
            }
            conclusive_breakdown = conclusive_df.groupby('test_type').agg(**conclusive_agg_spec)
            breakdown = breakdown.join(conclusive_breakdown, how='left')
...
class ClinicEnvKPIPreparer:
    ...
    def __init__(self, iot_df: pd.DataFrame):
        ...
        self.summary: Dict[str, Any] = {
            ...
            "avg_waiting_room_occupancy_overall_persons": np.nan, "rooms_noise_high_alert_latest_count": 0
        }
    ...
    def prepare(self) -> Dict[str, Any]:
        ...
        if 'timestamp' in self.df.columns and 'room_name' in self.df.columns:
            ...
            noise_series = latest.get('avg_noise_db', pd.Series(dtype=float))
            self.summary['rooms_noise_high_alert_latest_count'] = (noise_series > settings.ALERT_AMBIENT_NOISE_HIGH_DBA).sum()
        return self.summary
...
def get_clinic_summary_kpis(health_df_period: Optional[pd.DataFrame], source_context: str = "") -> Dict[str, Any]:
    """Factory function to calculate all summary KPIs for a clinic over a period."""
    if not isinstance(health_df_period, pd.DataFrame) or health_df_period.empty:
        return ClinicKPIPreparer(pd.DataFrame()).summary
    preparer = ClinicKPIPreparer(health_df_period)
...
def get_clinic_environmental_summary_kpis(iot_df_period: Optional[pd.DataFrame], source_context: str = "") -> Dict[str, Any]:
    ...
    if not isinstance(iot_df_period, pd.DataFrame) or iot_df_period.empty:
        return ClinicEnvKPIPreparer(pd.DataFrame()).summary
    preparer = ClinicEnvKPIPreparer(iot_df_period)
    return preparer.prepare()

# Placeholder stubs for other preparers not fully implemented in this refactoring for brevity
# In a full implementation, they would have their own classes similar to the above.
@st.cache_data(ttl=settings.CACHE_TTL_SECONDS_WEB_REPORTS, hash_funcs={pd.DataFrame: hash_dataframe_safe})
def get_chw_summary_kpis(health_df_daily: Optional[pd.DataFrame], for_date: Any, source_context: str = "") -> Dict[str, Any]:
    logger.info(f"({source_context}) Calculating CHW summary KPIs for date: {for_date} - (STUB)")
    # This would instantiate a CHWDailySummaryPreparer class
    return {"visits_count": 0, "high_ai_prio_followups_count": 0}

@st.cache_data(ttl=settings.CACHE_TTL_SECONDS_WEB_REPORTS, hash_funcs={pd.DataFrame: hash_dataframe_safe})
def get_district_summary_kpis(enriched_zone_df: Optional[pd.DataFrame], source_context: str = "") -> Dict[str, Any]:
    logger.info(f"({source_context}) Calculating district summary KPIs - (STUB)")
    # This would instantiate a DistrictKPIPreparer class
    return {"total_zones_in_df": 0, "total_population_district": 0}
