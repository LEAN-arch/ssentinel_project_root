...
def enrich_health_records_with_kpis(df: pd.DataFrame) -> pd.DataFrame:
    ...
    needed for efficient KPI calculations and analytics.
    """
    if df.empty:
        return df
...
def enrich_zone_data_with_aggregates(
    ...
    """Public factory function to enrich zone geographical data with aggregated metrics."""
    if not isinstance(zone_df, pd.DataFrame) or zone_df.empty:
        return pd.DataFrame(columns=['zone_id', 'geometry'])
    try:
...
class ZoneDataEnricher:
    ...
    def _aggregate_health_data(self) -> pd.DataFrame:
        ...
        summary = health_agg.groupby('zone_id').agg(
    ...
    def _finalize_schema(self, df: pd.DataFrame) -> pd.DataFrame:
        fill_values = {
            'avg_risk_score': np.nan, 'total_encounters': 0,
            'total_active_key_cases': 0, 'prevalence_per_1000_pop': 0.0,
            'zone_avg_co2_ppm': np.nan, 'zone_avg_pm25': np.nan
        }
