...
# Use a modern, clean template for all charts
...
def get_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    SME EXPANSION: The AI enrichment now includes a granular `risk_score`
    and geospatial coordinates for advanced analytics.
    """
    ...
    # The apply_ai_models function is assumed to add 'risk_score', 'lat', 'lon'
    enriched_df, _ = apply_ai_models(raw_health_df)

    # --- SME EXPANSION: Add AI-driven risk stratification for clearer insights ---
    if 'risk_score' in enriched_df.columns:
        enriched_df['risk_category'] = pd.cut(
            enriched_df['risk_score'],
            bins=RISK_BINS,
            labels=RISK_LABELS,
            right=False
        )
    else:
        # Fallback if AI model doesn't produce a risk score
        ...
        if not symptomatic.empty and 'risk_category' in symptomatic.columns:
            ...
            for risk_level, color in RISK_COLOR_MAP.items():
                fig_risk.add_trace(go.Bar(
                    x=[risk_distribution.get(risk_level, 0)], y=['Symptomatic'], name=risk_level,
                    orientation='h', marker_color=color
                ))
            fig_risk.update_layout(
                barmode='stack', title_text="Actionability: Focus testing on high-risk individuals", title_x=0.5,
                xaxis_title="Patient Count", yaxis_title="", legend_title="AI Risk Level",
                template=PLOTLY_TEMPLATE, height=200, margin=dict(t=40, b=10, l=10, r=10)
            )
...
def render_decision_support_tab(analysis_df: pd.DataFrame, forecast_df: pd.DataFrame):
    ...
    SME FIX: Added a safety check to ensure the forecast DataFrame is valid before use.
    """
    st.header("🎯 AI Predictive Analytics Hub")
    st.markdown("""
    This hub provides forward-looking intelligence to guide strategic decisions.
    - **Priority Alerts:** Identify patients needing immediate attention.
    - **Geospatial Hotspots:** Pinpoint high-risk areas for targeted interventions.
    - **Patient Load Forecast:** Predict future demand to optimize staffing.
    - **Supply Chain Forecast:** Anticipate test kit needs to prevent stock-outs.
    """)
    st.divider()
    ...
    with col2:
        with st.container(border=True):
            ...
            if len(forecast_df) < 10:
                ...
                # --- SME FIX: Check if the forecast was successful before trying to use it ---
                if not forecast.empty and 'yhat' in forecast.columns:
                    ...
                    # --- SME FIX: Handle the case where the forecast model failed ---
                    st.error("Forecast generation failed. The model may not have converged due to the selected data filters. Try a different date range or zone.")
...
def render_iot_wearable_tab(clinic_iot: pd.DataFrame, wearable_iot: pd.DataFrame, chw_filter: str, health_df: pd.DataFrame):
    ...
    with col1:
        ...
        if not clinic_iot.empty:
            ...
            st.caption("High CO₂ levels can indicate poor ventilation, a risk factor for airborne diseases like TB.")
        else:
        ...
        if not wearable_iot.empty and chw_filter == "All CHWs":
            ...
            if not chw_burnout_df.empty: # Add check to avoid division by zero
    ...
    with col2:
        ...
        if not wearable_iot.empty:
            ...
            st.caption("Monitors team stress levels, which can impact performance and patient care.")
        else:
...
def main():
    ...
    with tabs[0]:
        ...
        st.markdown("Use the tabs below to monitor the performance of each key public health screening program, now enhanced with AI risk profiles.")
