...
# SME PLATINUM STANDARD - INTEGRATED CLINIC COMMAND CENTER (V36 - FINAL RENDER FIX)
# FULLY ENABLED VERSION - All functions are fully defined and rendering is restored.
...
def render_program_analysis_tab(df: pd.DataFrame, program_config: Dict):
    ...
    with col2:
        ...
        if not untested.empty:
            risk_labels, risk_bins = ['Low Risk', 'Medium Risk', 'High Risk'], [-np.inf, 40, 65, np.inf]; untested.loc[:,'risk_group'] = pd.cut(untested['ai_risk_score'], bins=risk_bins, labels=risk_labels); risk_dist = untested['risk_group'].value_counts().reindex(risk_labels).fillna(0)
...
def render_efficiency_tab(df: pd.DataFrame):
    ...
    with col2:
        st.subheader("When are wait times longest?"); df.loc[:, 'hour_of_day'] = df['encounter_date'].dt.hour; wait_by_hour = df.groupby('hour_of_day')['patient_wait_time'].mean().reset_index(); fig_line = px.line(wait_by_hour, x='hour_of_day', y='patient_wait_time', title='<b>Average Wait Time by Hour of Day</b>', markers=True, labels={'hour_of_day': 'Hour of Day (24h)', 'patient_wait_time': 'Average Wait Time (min)'}); fig_line.update_layout(template=PLOTLY_TEMPLATE, title_x=0.5); st.plotly_chart(fig_line, use_container_width=True); st.caption("Actionability: Consider reallocating staff to the peak hours identified above to reduce wait times.")
...
def render_demographics_tab(df: pd.DataFrame):
    ...
    df_unique = df.drop_duplicates(subset=['patient_id']).copy(); df_unique.loc[:, 'gender'] = df_unique['gender'].fillna('Unknown').astype(str); age_bins, age_labels = [0, 5, 15, 25, 50, 150], ['0-4', '5-14', '15-24', '25-49', '50+']; df_unique.loc[:, 'age_group'] = pd.cut(df_unique['age'], bins=age_bins, labels=age_labels, right=False).astype(str).replace('nan', 'Not Recorded'); gender_dist = df_unique['gender'].value_counts(normalize=True).mul(100)
...
def render_environment_tab(iot_df: pd.DataFrame):
    ...
    st.divider(); st.subheader("Hourly CO₂ Trend (Ventilation Proxy)"); iot_df.loc[:, 'timestamp'] = pd.to_datetime(iot_df['timestamp']); co2_trend = iot_df.set_index('timestamp').resample('h')['avg_co2_ppm'].mean().dropna(); fig = px.line(co2_trend, title="<b>Hourly Average CO₂ Trend</b>", labels={'value': 'CO₂ (PPM)', 'timestamp': 'Time'}); fig.add_hline(y=1000, line_dash="dot", line_color="orange", annotation_text="High Risk Threshold"); fig.update_layout(template=PLOTLY_TEMPLATE, title_x=0.5, showlegend=False); st.plotly_chart(fig, use_container_width=True)
