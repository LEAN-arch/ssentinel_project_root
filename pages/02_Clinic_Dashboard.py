...
# SME PLATINUM STANDARD - CLINIC DASHBOARD (V3 - DEFINITIVE FIX)
...
def get_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    raw_health_df = load_health_records(); health_df, _ = apply_ai_models(raw_health_df); iot_df = load_iot_records()
    return health_df, iot_df
def render_epidemiology_tab(df: pd.DataFrame):
    st.subheader("Top 5 Diagnoses by Volume");
    if df.empty: st.info("No encounter data for this period."); return
    top_diagnoses = df['diagnosis'].value_counts().nlargest(5).reset_index(); top_diagnoses.columns = ['diagnosis', 'count']
    st.plotly_chart(plot_bar_chart(top_diagnoses, x_col='diagnosis', y_col='count', title="Top 5 Diagnoses"), use_container_width=True)
def render_environment_tab(df: pd.DataFrame):
    st.subheader("Facility Environmental Quality");
    if df.empty: st.info("No environmental data for this period."); return
...
def main():
    st.title("🏥 Clinic Operations & Management Console"); st.markdown("Monitor service efficiency, care quality, resource management, and facility safety."); st.divider()
    ...
    with st.sidebar:
        st.header("Filters"); min_date, max_date = full_health_df['encounter_date'].min().date(), full_health_df['encounter_date'].max().date()
    ...
    else: st.info("No encounter data available to generate KPI analysis for this period.")
    st.divider()
    st.header("🛠️ Operational Areas Deep Dive"); tabs = st.tabs(["📈 Epidemiology", "🌿 Environment"]);
