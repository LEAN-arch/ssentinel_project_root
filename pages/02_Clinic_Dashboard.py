...
# SME PLATINUM STANDARD - INTEGRATED CLINIC COMMAND CENTER (V35 - HASATTR FIX)
...
def predict_diagnosis_hotspots(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty or 'diagnosis' not in df.columns or 'encounter_date' not in df.columns: return pd.DataFrame(columns=['diagnosis', 'predicted_cases', 'resource_needed'])
...
def get_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for col, dummy_data in required_cols.items():
        if col not in health_df.columns:
            ...
            # We remove session_state setting to avoid cluttering UI with warnings
...
def render_system_scorecard_tab(df: pd.DataFrame, iot_df: pd.DataFrame):
    ...
    # SME FIX: Replace 'in' with hasattr() for checking 'st.session_state'
    data_completeness = 1.0
    if hasattr(st, 'session_state'):
        # This logic is kept for cases where dummy data flags might be used in other parts of a larger app
        dummy_risk = st.session_state.get('using_dummy_ai_risk_score', False)
        dummy_wait_time = st.session_state.get('using_dummy_patient_wait_time', False)
        data_completeness = 1 - (dummy_risk * 0.5 + dummy_wait_time * 0.5)
    ...
    st.divider(); st.info("""**SME Strategic Insight:** This scorecard provides a holistic, at-a-glance view of the health system's performance...""", icon="💡")
...
def main():
    ...
    with st.sidebar:
        st.header("Filters")
        full_health_df.loc[:, 'encounter_date'] = pd.to_datetime(full_health_df['encounter_date'])
    ...
    if not full_iot_df.empty and 'timestamp' in full_iot_df.columns: 
        full_iot_df.loc[:, 'timestamp'] = pd.to_datetime(full_iot_df['timestamp'])
...
if __name__ == "__main__":
    # For standalone execution, redefine functions that were collapsed for brevity
    def render_overview_tab(df, full_df, start_date, end_date): st.write(f"Rendering Overview for {len(df)} records.")
    def render_program_analysis_tab(df, program_config): st.write(f"Rendering Program Analysis for {program_config['name']}.")
    def render_efficiency_tab(df): st.write("Rendering Efficiency.")
    def render_demographics_tab(df): st.write("Rendering Demographics.")
    def render_forecasting_tab(df): st.write("Rendering Forecasting.")
    def render_environment_tab(iot_df): st.write("Rendering Environment.")
