...
def predict_diagnosis_hotspots(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df.empty: return pd.DataFrame(columns=['diagnosis', 'predicted_cases'])
    ...
    if len(weekly_counts) < 2: last_week_avg = weekly_counts.iloc[-1]
    else: last_week_avg = weekly_counts.iloc[-2:].mean()
    predictions = {}
...
def get_data() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if raw_health_df.empty: return pd.DataFrame(), iot_df
    health_df, _ = apply_ai_models(raw_health_df)
    if 'patient_wait_time' not in health_df.columns: health_df['patient_wait_time'] = np.random.uniform(5, 60, len(health_df))
    if 'consultation_duration' not in health_df.columns: health_df['consultation_duration'] = np.random.uniform(10, 30, len(health_df))
...
def render_overview_tab(df: pd.DataFrame, full_df: pd.DataFrame, start_date: date, end_date: date):
    period_str = f"{start_date:%d %b %Y} - {end_date:%d %b %Y}"
    st.header(f"🚀 Clinic Overview")
    ...
    unique_patients, prev_unique_patients = df['patient_id'].nunique(), prev_df['patient_id'].nunique()
    ...
    col3.metric("High-Risk Patients", f"{high_risk_patients:,}", f"{high_risk_patients - prev_high_risk:+,}" if prev_high_risk > 0 else "N/A", delta_color="inverse", help=f"Score ≥ {settings.ANALYTICS.risk_score_moderate_threshold}")
    st.divider()

    col1, col2 = st.columns(2, gap="large")
    with col1:
        st.subheader("Weekly Diagnosis Trends")
        st.markdown("Spot historical trends in the most common diagnoses.")
        ...
        if not heatmap_data.empty:
            ...
            fig = px.imshow(heatmap_data.T, text_auto=True, aspect="auto",
                            ...
                            title="Weekly Case Volume by Diagnosis")
    ...
    with col2:
        ...
        if not df_top.empty and not heatmap_data.empty:
            ...
            fig.update_layout(
                barmode='group', title_text="What's Coming Next Week?<br><sup>Actual vs. AI-Predicted Cases</sup>",
                xaxis_title=None, yaxis_title='Case Count', template=PLOTLY_TEMPLATE, title_x=0.5,
    ...
    with col1:
        ...
        if not untested.empty:
            ...
            fig_donut.update_layout(
                ...
                template=PLOTLY_TEMPLATE, showlegend=True, title_x=0.5,
                legend=dict(orientation="h", yanchor="bottom", y=-0.2, xanchor="center", x=0.5),
                annotations=[dict(text='High-Risk<br>Untested', x=0.5, y=0.5, font_size=16, showarrow=False)]
            )
...
def render_demographics_tab(df: pd.DataFrame):
    ...
    if analysis_type == "Patient Volume":
        ...
        if not demo_counts.empty:
            fig = plot_bar_chart(demo_counts, x_col='age_group', y_col='count', color='gender', barmode='group', title="Patient Volume by Age and Gender", x_title="Age Group", y_title="Number of Unique Patients", category_orders={'age_group': age_labels + ['Not Recorded']}, color_discrete_map=GENDER_COLORS)
            fig.update_layout(template=PLOTLY_TEMPLATE, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
            st.plotly_chart(fig, use_container_width=True)
        ...
        if not risk_by_demo.empty:
            fig = plot_bar_chart(risk_by_demo, x_col='age_group', y_col='ai_risk_score', color='gender', barmode='group', title="Average AI Risk Score by Age and Gender", x_title="Age Group", y_title="Average AI Risk Score", category_orders={'age_group': age_labels + ['Not Recorded']}, color_discrete_map=GENDER_COLORS)
            fig.update_layout(template=PLOTLY_TEMPLATE, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
        ...
        if not high_risk_df.empty:
            ...
            fig = px.treemap(
                ...
                hover_data={'ai_risk_score': ':.1f'}
            )
            fig.update_traces(textinfo="label+value", marker_line_color="white")
            fig.update_layout(template=PLOTLY_TEMPLATE, title_x=0.5, margin=dict(t=50, l=25, r=25, b=25))
            st.plotly_chart(fig, use_container_width=True)
        else:
...
def render_forecasting_tab(df: pd.DataFrame):
    ...
    with col1:
        ...
        if 'yhat' in encounter_fc.columns:
            ...
            st.warning("Could not generate forecast with the available data. Try a broader date range.")
    ...
    with col2:
        ...
        if 'yhat' in encounter_fc.columns:
            ...
            st.metric(f"Total Predicted Visits ({forecast_days} days)", f"{total_predicted_patients:,.0f}")
            ...
            fig_gauge = go.Figure(go.Indicator(
                ...
                gauge = {'axis': {'range': [None, 120], 'tickwidth': 1, 'tickcolor': "darkblue"},
                         'bar': {'color': "#2c3e50"},
                         'steps' : [{'range': [0, 80], 'color': "#28a745"}, {'range': [80, 100], 'color': "#ffc107"}, {'range': [100, 120], 'color': "#dc3545"}],
                         'threshold' : {'line': {'color': "#8B0000", 'width': 4}, 'thickness': 0.9, 'value': 100}}
            ))
            fig_gauge.update_layout(height=250, margin=dict(t=50, b=10, l=30, r=30))
...
def render_environment_tab(iot_df: pd.DataFrame):
    ...
    st.plotly_chart(plot_line_chart(co2_trend, "Hourly Average CO₂ Trend<br><sup>Proxy for Clinic Ventilation</sup>", y_title="CO₂ (PPM)"), use_container_width=True)
...
def render_efficiency_tab(df: pd.DataFrame):
    ...
    col1, col2, col3 = st.columns(3)
    avg_wait_time, avg_consult_duration = df['patient_wait_time'].mean(), df['consultation_duration'].mean()
    ...
    with col1:
        ...
        fig_hist.update_layout(title_x=0.5, yaxis_title="Number of Patients")
    ...
    with col2:
        ...
        fig_scatter = px.scatter(
            ...
            color="ai_risk_score", color_continuous_scale="Viridis",
...
def main():
    ...
    with tabs[0]: render_overview_tab(period_health_df, full_health_df, start_date, end_date)
    ...
    with tabs[-4]: render_demographics_tab(period_health_df)
    with tabs[-3]: render_forecasting_tab(full_health_df)
    with tabs[-2]: render_efficiency_tab(period_health_df)
