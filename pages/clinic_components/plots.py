...
def create_empty_figure(title: str, height: Optional[int] = None, message: str = "No data available.") -> go.Figure:
    """Creates a blank Plotly figure with a message."""
    final_height = height or _get_setting('WEB_PLOT_DEFAULT_HEIGHT', 400)
    fig = go.Figure()
    fig.update_layout(
        title_text=f'<b>{title}</b>', height=final_height,
        xaxis=dict(visible=False), yaxis=dict(visible=False),
        annotations=[dict(text=message, xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False, font_size=14)]
    )
    return fig
...
def plot_annotated_line_chart(
    ...
    fig = px.line(df, x='x', y='y', title=f'<b>{chart_title}</b>', markers=True)
    
    y_format = ',d' if y_values_are_counts else ',.1f'
    fig.update_traces(hovertemplate=f'<b>%{{x|%b %d, %Y}}</b><br>{y_axis_label}: %{{y:{y_format}}}<extra></extra>')
...
def plot_bar_chart(
    ...
    # FIXED: Use %{y} instead of %{text} to display the bar's value, and use correct d3 format string.
    if y_values_are_counts:
