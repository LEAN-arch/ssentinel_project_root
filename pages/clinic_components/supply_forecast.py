...
# --- Forecasting Strategies (Refactored for maintainability and performance) ---
...
class SimpleAggregateForecaster:
    ...
    def forecast(self) -> Tuple[pd.DataFrame, List[str]]:
        ...
        # FIX: Ensure stockout date is handled safely for non-finite values
        ...
        # FIX: Replace non-finite 'days_of_supply_remaining' with np.nan to ensure numeric type
...
class AIAdvancedForecaster(SimpleAggregateForecaster):
    """Placeholder for a more advanced forecasting model."""
    def __init__(self, historical_df: pd.DataFrame, items_to_forecast: List[str]):
...
class ClinicSupplyForecastPreparer:
    ...
    def _find_items_to_forecast(self) -> List[str]:
        ...
        items = [item for item in self.df_historical[COL_ITEM].dropna().unique() if re.search(drug_pattern, item, re.IGNORECASE)]
        if not items: self.notes.append("No items matching keywords found in data.")
        return items
    ...
    def prepare(self) -> Dict[str, Any]:
        ...
        if self.use_ai_model: forecaster = AIAdvancedForecaster(self.df_historical, items_to_forecast)
        else: forecaster = SimpleAggregateForecaster(self.df_historical, items_to_forecast)
