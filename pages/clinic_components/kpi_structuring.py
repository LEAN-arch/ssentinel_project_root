...
class KPIStructurer:
    ...
    def _build_kpi(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"title": config["title"], "value_str": f"{formatted_value}{units}" if units == "%" and pd.notna(value) else formatted_value, "units": units if units != "%" else "", "icon": config["icon"], "status_level": status, "help_text": help_text}
    ...
    def structure_disease_and_supply_kpis(self) -> List[Dict[str, Any]]:
        ...
        kpi_defs.append(stockout_conf)
        return [self._build_kpi(conf) for conf in kpi_defs]

# --- Public Factory Functions ---
# DEFINITIVE FIX: The parameter names are changed to match what the dashboard is calling them with.
def structure_main_clinic_kpis(kpis_summary: Optional[Dict[str, Any]], **kwargs) -> List[Dict[str, Any]]:
    ...
    structurer = KPIStructurer(kpis_summary)
