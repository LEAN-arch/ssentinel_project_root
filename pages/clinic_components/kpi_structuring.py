...
# --- DEFINITIVE FIX ---
# The KPI structuring functions are robust and will return empty lists if there is no data.
# The render_kpi_row function handles empty lists gracefully by rendering nothing.
# This prevents incorrect warnings from showing on valid "no data" periods.
...
if not main_kpis and not disease_kpis and not period_health_df.empty:
    st.warning("Could not generate service performance KPIs. The underlying summary data might be incomplete.")
elif not main_kpis and not disease_kpis and period_health_df.empty:
