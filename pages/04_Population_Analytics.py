...
# SME PLATINUM STANDARD - POPULATION HEALTH ANALYTICS
...
with st.sidebar:
    ...
    start_date, end_date = st.date_input(
        "Select Date Range:",
        value=(max(min_date, max_date - timedelta(days=89)), max_date),
...
with cols[1]: render_custom_kpi("Avg. Risk Score", df_filtered['ai_risk_score'].mean(), "0-100 scale")
high_risk_count = (df_filtered['ai_risk_score'] >= settings.ANALYTICS.risk_score_moderate_threshold).sum()
...
with cols[3]: render_custom_kpi("Median Patient Age", df_filtered['age'].median(), "Years")
st.divider()
...
with tab3:
    ...
    if 'geometry' not in zone_df.columns:
        st.warning("Geospatial data is unavailable.")
    else:
        geojson_data = {"type": "FeatureCollection", "features": [
            {"type": "Feature", "properties": {"zone_id": row['zone_id']}, "geometry": row['geometry']}
            for _, row in zone_df.iterrows() if pd.notna(row.get('geometry'))
        ]}
