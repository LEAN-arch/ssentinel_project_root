...
# SME PLATINUM STANDARD - INTEGRATED LOGISTICS & CAPACITY DASHBOARD (V20 - DEFINITIVE AND FULLY FUNCTIONAL)
...
SUPPLY_CATEGORIES = {
    "Medications": { "items": settings.KEY_SUPPLY_ITEMS, "data_col": "medication_prescribed", "stock_col": "item_stock_agg_zone", "date_col": "encounter_date"},
    "Diagnostic Tests": { "items": list(settings.KEY_TEST_TYPES.keys()), "data_col": "test_type", "stock_col": "test_kit_stock", "date_col": "encounter_date"}
}

# --- Data Loading & Caching ---
@st.cache_data(ttl=3600, show_spinner="Loading all operational data...")
def get_data() -> dict:
    """Loads all data for the dashboard. Now relies on loaders.py for robustness."""
...
def render_supply_kpi_dashboard(summary_df: pd.DataFrame):
    ...
    if summary_df.empty:
        st.warning("Could not generate forecasts. Selected items may have insufficient historical data."); return
...
def render_reorder_analysis(summary_df: pd.DataFrame):
    ...
    if summary_df.empty: st.info("Run a forecast to see reorder recommendations."); return
...
def render_cold_chain_tab(iot_df: pd.DataFrame):
    ...
    if iot_df.empty: st.warning("No cold chain IoT data available."); return
