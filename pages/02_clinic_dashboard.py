...
def get_kpi_analysis_table(full_df: pd.DataFrame, start_date: date, end_date: date) -> pd.DataFrame:
    ...
    for name, (key, trend_logic) in kpi_defs.items():
        ...
        # The key for trend data might be different from the KPI key (e.g., without _perc)
        trend_key = key.replace("_perc", "").replace("_days", "")
...
with tab4:
    ...
    if period_health_df.empty: st.info("No data for patient analysis.")
        ...
        if not risk_df.empty:
            ...
            fig = plot_bar_chart(risk_df, x_col='age', y_col='ai_risk_score', color='Risk Category', title="Patient Risk Score vs. Age", x_axis_title="Patient Age", y_axis_title="AI Risk Score", barmode='overlay', y_values_are_counts=False)
