...
try:
    ...
    # Local components are defined within this script to prevent import errors.
...
class _KPIStructurer:
    ...
    def __init__(self, kpis_summary: Optional[Dict[str, Any]]):
        ...
        self._DISEASE_KPI_DEFS = [
            # This will be populated dynamically
        ]
        key_tests = getattr(settings, 'KEY_TEST_TYPES_FOR_ANALYSIS', {})
        for test_name, config in key_tests.items():
            if isinstance(config, dict):
                self._DISEASE_KPI_DEFS.append({"title": f"{config.get('display_name', test_name)} Positivity", "source_key": f"test_summary_details.{test_name}.positive_rate_perc", "target_value": float(config.get("target_max_positivity_pct", 10.0)), "units": "%", "icon": config.get("icon", "🔬"), "help_template": "Positivity rate. Target: <{target:.1f}%.", "status_logic": "lower_is_better", "precision": 1})
        self._DISEASE_KPI_DEFS.append({"title": "Key Drug Stockouts", "source_key": "key_drug_stockouts_count", "target_setting": "TARGET_DRUG_STOCKOUTS", "default_target": 0, "units": "items", "icon": "💊", "help_template": "Key drugs with <{days_remaining} days of stock. Target: {target}.", "status_logic": "lower_is_better_count", "is_count": True, "precision": 0})
    ...
    def _build_kpi(self, conf):
        ...
        is_count = conf.get("is_count", False)
        precision = conf.get("precision", 1)
        
        if pd.isna(value): val_str = "N/A"
        else: val_str = f"{int(value_num):,}" if is_count else f"{value_num:.{precision}f}"
        
        help_text = conf["help_template"].format(target=target, days_remaining=getattr(settings, 'CRITICAL_SUPPLY_DAYS_REMAINING', 7))
        return {"title": conf["title"], "value_str": f"{val_str}{units}" if units == "%" else val_str, "units": units if units != "%" else "", "icon": conf["icon"], "status_level": status, "help_text": help_text}

    def structure_main_kpis(self): return [self._build_kpi(c) for c in self._MAIN_KPI_DEFS]
    def structure_disease_and_supply_kpis(self): return [self._build_kpi(conf) for conf in self._DISEASE_KPI_DEFS]
...
#... (The rest of the file remains the same, restoring full functionality)
st.divider()
