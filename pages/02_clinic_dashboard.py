    ...
    if period_health_df.empty:
        ...
        with col1:
            ...
            if not tat_df.empty:
                ...
                fig.update_layout(
                    title_text="<b>Average Turnaround Time (TAT) by Test</b>",
                    yaxis={'categoryorder':'total ascending'}, xaxis_title="Average Days"
                )
                st.plotly_chart(fig, use_container_width=True)
        with col2:
...
with tab3:
    ...
    forecastable_items = sorted([
        item for item in full_health_df['item'].dropna().unique() 
    ...
    if not forecastable_items:
        ...
        if selected_items:
            ...
            if not forecast_df.empty:
                ...
                fig.update_layout(
                    title_text="<b>Forecasted Days of Supply for Critical Items</b>",
                    xaxis_title="Date", yaxis_title="Days of Supply Remaining", legend_title="Item", yaxis_tickformat='d'
                )
                st.plotly_chart(fig, use_container_width=True)
            else:
...
with tab4:
    ...
    if period_health_df.empty:
        ...
        if not risk_df.empty:
            ...
            fig.update_layout(
                title_text="<b>Patient Risk Score vs. Age</b>",
                xaxis_title="Patient Age", yaxis_title="AI Risk Score",
        ...
        if not flagged_patients.empty:
            ...
            st.dataframe(
                flagged_patients[display_cols].head(20),
                use_container_width=True, hide_index=True,
                column_config={"ai_risk_score": st.column_config.ProgressColumn("Risk Score", format="%d", min_value=0, max_value=100)}
            )
        else:
...
with tab5:
    ...
    if period_iot_df.empty:
        ...
        st.dataframe(latest_readings[display_cols], use_container_width=True, hide_index=True,
                     column_config={
                         "timestamp": st.column_config.DatetimeColumn("Last Reading", format="YYYY-MM-DD HH:mm"),
                         "avg_temp_celsius": st.column_config.NumberColumn("Temp (°C)")
                     })
