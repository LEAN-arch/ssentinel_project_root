...
st.markdown("ℹ️ Data loading failed. Cannot display daily performance snapshot.")
elif daily_activity_df.empty:
st.markdown("ℹ️ No activity data for selected date/filters for daily performance snapshot.")
else:
...
prio_threshold = _get_setting('FATIGUE_INDEX_HIGH_THRESHOLD', 80)
prio_status = "ACCEPTABLE" if prio_followups <= 2 else ("MODERATE_CONCERN" if prio_followups <= 5 else "HIGH_CONCERN")
render_kpi_card(title="High Prio Follow-ups", value_str=str(prio_followups), icon="🎯", status_level=prio_status, help_text=f"Patients needing urgent follow-up (AI prio score ≥ {prio_threshold}).")
with kpi_cols[2]:
...
with kpi_cols[3]:
fever_threshold = _get_setting('ALERT_BODY_TEMP_HIGH_FEVER_C', 39.5)
...
chw_tasks = []
if not data_load_successful:
st.markdown("ℹ️ Data loading failed. Cannot display alerts or tasks.")
elif daily_activity_df.empty:
...
if not data_load_successful:
st.markdown("ℹ️ Data loading failed. Cannot display local epi signals.")
elif daily_activity_df.empty:
...
if not data_load_successful:
st.markdown("ℹ️ Data loading failed earlier. Cannot display activity trends.")
elif trend_activity_df.empty:
st.markdown("ℹ️ No historical data available for the selected trend period and/or filters to calculate trends.")
else:
