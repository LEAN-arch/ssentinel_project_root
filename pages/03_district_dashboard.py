...
if isinstance(enriched_zone_df_display, pd.DataFrame) and not enriched_zone_df_display.empty:
    ...
    kpi_data.append({
        "icon": "👥", "title": "Total District Population", "metric": total_pop,
    ...
    kpi_data.append({
        "icon": "⚠️", "title": "Avg. AI Risk Score (Pop. Weighted)", "metric": weighted_risk_score,
    ...
    kpi_data.append({
        "icon": "🔬", "title": f"{key_disease_name} Prevalence (/1k Pop.)", "metric": prevalence_kpi,
    ...
    kpi_data.append({
        "icon": "🚩", "title": "High-Risk Zones Count", "metric": high_risk_zones,
        "help": help_text_risk
    })
