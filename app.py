...
# This is crucial for Python to find the 'config' package and other project modules.
if str(_project_root_dir) not in sys.path:
    sys.path.insert(0, str(_project_root_dir))
    print(f"DEBUG (app.py): Added project root to sys.path: {_project_root_dir}", file=sys.stderr)
elif sys.path[0] != str(_project_root_dir): # If already in path, ensure it's first
    try:
        sys.path.remove(str(_project_root_dir))
    except ValueError:
        pass # Not found, insert will add it
    sys.path.insert(0, str(_project_root_dir))
...
# Explicitly print sys.path BEFORE attempting to import settings
print(f"DEBUG (app.py): sys.path BEFORE importing settings = {sys.path}", file=sys.stderr)
...
try:
    ...
    print(f"Is 'config' directory directly under '{_project_root_dir}' with an __init__.py?", file=sys.stderr)
    sys.exit(1) 
...
valid_log_levels_app_cfg = {"DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"}
log_level_app_str_cfg = str(settings.LOG_LEVEL).upper()
if log_level_app_str_cfg not in valid_log_levels_app_cfg:
    print(f"WARN (app.py): Invalid LOG_LEVEL '{log_level_app_str_cfg}'. Using INFO.", file=sys.stderr); log_level_app_str_cfg = "INFO"
logging.basicConfig(level=getattr(logging, log_level_app_str_cfg, logging.INFO), format=settings.LOG_FORMAT,
...
try:
    ...
    if version.parse(streamlit.__version__) >= version.parse("1.30.0"):
        STREAMLIT_VERSION_GE_1_30 = True
...
except Exception as e_st_ver_cfg: logger.warning(f"Could not accurately determine Streamlit version/features: {e_st_ver_cfg}")

# --- Dependency Check for Plotly (Example) ---
if not importlib.util.find_spec("plotly"):
    logger.warning("Plotly library not found. Visualization features dependent on Plotly may fail.")
    # Optionally, st.warning("Plotly library not found...") if critical for UI.

# --- Page Configuration ---
page_icon_path_obj_app_main_cfg = Path(settings.APP_LOGO_SMALL_PATH) 
final_page_icon_str_app_main_cfg: str = str(page_icon_path_obj_app_main_cfg) if page_icon_path_obj_app_main_cfg.is_file() else "🌍"
if final_page_icon_str_app_main_cfg == "🌍": logger.warning(f"Page icon not found at '{page_icon_path_obj_app_main_cfg}'. Using '🌍'.")
st.set_page_config(
    page_title=f"{settings.APP_NAME} - System Overview", page_icon=final_page_icon_str_app_main_cfg,
...
except Exception as e_theme_main_app_cfg_final: logger.error(f"Error applying Plotly theme: {e_theme_main_app_cfg_final}", exc_info=True); st.error("Error applying visualization theme.")
...
header_cols_app_ui_final_cfg_val = st.columns([0.12, 0.88])
with header_cols_app_ui_final_cfg_val[0]:
...
with header_cols_app_ui_final_cfg_val[1]: st.title(html.escape(settings.APP_NAME)); st.subheader("Transforming Data into Lifesaving Action at the Edge")
st.divider()
...
num_cols_core_principles_final_v4 = min(len(core_principles_main_app_v4), 2)
if num_cols_core_principles_final_v4 > 0:
    cols_core_principles_ui_final_v4 = st.columns(num_cols_core_principles_final_v4)
    for idx_core_final_v4, (title_core_final_v4, desc_core_final_v4) in enumerate(core_principles_main_app_v4):
        with cols_core_principles_ui_final_v4[idx_core_final_v4 % num_cols_core_principles_final_v4]:
            st.markdown(f"##### {html.escape(title_core_final_v4)}"); st.markdown(f"<small>{html.escape(desc_core_final_v4)}</small>", unsafe_allow_html=True)
...
# ASSUMING FILENAMES IN pages/ ARE NOW PREFIXED FOR ORDER, e.g., 01_chw_dashboard.py
role_navigation_config_app_final_cfg_list = [
...
num_nav_cols_final_app_cfg_val_ui_v2 = min(len(role_navigation_config_app_final_cfg_list), 2)
if num_nav_cols_final_app_cfg_val_ui_v2 > 0:
    nav_cols_ui_final_app_cfg_val_ui_v2 = st.columns(num_nav_cols_final_app_cfg_val_ui_v2)
    current_col_idx_nav_final_cfg_val_ui_v2 = 0
    for nav_item_final_app_cfg_item_val_v2 in role_navigation_config_app_final_cfg_list:
        page_link_target_app_cfg_item_val_v2 = nav_item_final_app_cfg_item_val_v2['page_filename'] 
        physical_page_full_path_app_cfg_item_val_v2 = pages_directory_obj_app_cfg / nav_item_final_app_cfg_item_val_v2["page_filename"]
        if not physical_page_full_path_app_cfg_item_val_v2.exists():
            logger.warning(f"Navigation page file for '{nav_item_final_app_cfg_item_val_v2['title']}' not found: {physical_page_full_path_app_cfg_item_val_v2}")
            continue
        with nav_cols_ui_final_app_cfg_val_ui_v2[current_col_idx_nav_final_cfg_val_ui_v2 % num_nav_cols_final_app_cfg_val_ui_v2]:
            container_args_final_app_cfg_val_v2 = {"border": True} if STREAMLIT_VERSION_GE_1_30 else {}
            with st.container(**container_args_final_app_cfg_val_v2):
                st.subheader(f"{nav_item_final_app_cfg_item_val_v2['icon']} {html.escape(nav_item_final_app_cfg_item_val_v2['title'])}")
                st.markdown(f"<small>{nav_item_final_app_cfg_item_val_v2['desc']}</small>", unsafe_allow_html=True)
                link_label_final_app_cfg_val_v2 = f"Explore {nav_item_final_app_cfg_item_val_v2['title'].split('(')[0].split('View')[0].strip()} View"
                if STREAMLIT_PAGE_LINK_AVAILABLE:
                    link_kwargs_final_app_cfg_val_v2 = {"use_container_width": True} if STREAMLIT_VERSION_GE_1_30 else {}
                    st.page_link(page_link_target_app_cfg_item_val_v2, label=link_label_final_app_cfg_val_v2, icon="➡️", **link_kwargs_final_app_cfg_val_v2)
                else: 
                    st.markdown(f'<a href="{nav_item_final_app_cfg_item_val_v2["page_filename"]}" target="_self" style="display:block;text-align:center;padding:0.5em;background-color:var(--sentinel-color-action-primary);color:white;border-radius:4px;text-decoration:none;">{link_label_final_app_cfg_val_v2} ➡️</a>', unsafe_allow_html=True)
            st.markdown("<div style='margin-bottom:0.5rem;'></div>", unsafe_allow_html=True)
        current_col_idx_nav_final_cfg_val_ui_v2 += 1
st.divider()

st.header(f"{settings.APP_NAME} - Key Capabilities Reimagined")
# Full capabilities descriptions from prompt
capabilities_data_app_final_cfg_full_v2 = [
...
num_cap_cols_final_app_cfg_val_final_v2 = min(len(capabilities_data_app_final_cfg_full_v2), 3)
if num_cap_cols_final_app_cfg_val_final_v2 > 0:
    cap_cols_ui_final_app_cfg_val_final_v2 = st.columns(num_cap_cols_final_app_cfg_val_final_v2)
    for i_cap_final_cfg_final_v2, (cap_t_final_cfg_final_v2, cap_d_final_cfg_final_v2) in enumerate(capabilities_data_app_final_cfg_full_v2):
        with cap_cols_ui_final_app_cfg_val_final_v2[i_cap_final_cfg_final_v2 % num_cap_cols_final_app_cfg_val_final_v2]: 
            st.markdown(f"##### {html.escape(cap_t_final_cfg_final_v2)}"); st.markdown(f"<small>{html.escape(cap_d_final_cfg_final_v2)}</small>", unsafe_allow_html=True)
...
st.sidebar.divider()
glossary_filename_sidebar_cfg_final_val_v2 = "05_glossary_page.py" 
glossary_link_target_sidebar_cfg_final_val_v2 = glossary_filename_sidebar_cfg_final_val_v2
glossary_physical_path_final_sb_cfg_final_val_v2 = pages_directory_obj_app_cfg / glossary_filename_sidebar_cfg_final_val_v2
if glossary_physical_path_final_sb_cfg_final_val_v2.exists():
    if STREAMLIT_PAGE_LINK_AVAILABLE: st.sidebar.page_link(glossary_link_target_sidebar_cfg_final_val_v2, label="📜 System Glossary", icon="📚")
    else: st.sidebar.markdown(f'<a href="{glossary_filename_sidebar_cfg_final_val_v2}" target="_self">📜 System Glossary</a>', unsafe_allow_html=True)
else: logger.warning(f"Glossary page for sidebar (expected: {glossary_physical_path_final_sb_cfg_final_val_v2}) not found.")
st.sidebar.divider()
