...
elif sys.path[0] != project_root_str_app: # If already in path, ensure it's first
    try:
...
except ImportError as e_cfg_app_final_corrected_v3: # Renamed exception var
    print(f"FATAL_APP_PY (L53): STILL FAILED to import config.settings: {e_cfg_app_final_corrected_v3}", file=sys.stderr)
    print(f"FINAL sys.path at import failure: {sys.path}", file=sys.stderr)
    config_package_path_check_v3 = _project_root_dir_app / 'config'
    config_init_path_check_v3 = config_package_path_check_v3 / '__init__.py'
    config_settings_path_check_v3 = config_package_path_check_v3 / 'settings.py'
    print(f"Check: Does '{config_package_path_check_v3}' exist and is it a directory? {config_package_path_check_v3.is_dir()}", file=sys.stderr)
    print(f"Check: Does '{config_init_path_check_v3}' exist? {config_init_path_check_v3.is_file()}", file=sys.stderr)
    print(f"Check: Does '{config_settings_path_check_v3}' exist? {config_settings_path_check_v3.is_file()}", file=sys.stderr)
    sys.exit(1) 
except AttributeError as e_attr_settings_final_corrected_v3: # Renamed exception var
    print(f"FATAL_APP_PY (L65): AttributeError on 'config.settings' (likely circular import OR settings.py has an internal error OR sys.path still incorrect): {e_attr_settings_final_corrected_v3}", file=sys.stderr)
    print(f"FINAL sys.path at attribute error: {sys.path}", file=sys.stderr)
    sys.exit(1)
except Exception as e_generic_cfg_final_corrected_v3: # Renamed exception var
    print(f"FATAL_APP_PY (L70): Generic error during 'config.settings' import: {e_generic_cfg_final_corrected_v3}", file=sys.stderr)
...
valid_log_levels_app_final_cfg_v4 = {"DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"} # Renamed var
log_level_app_str_final_cfg_v4 = str(settings.LOG_LEVEL).upper()
if log_level_app_str_final_cfg_v4 not in valid_log_levels_app_final_cfg_v4:
    print(f"WARN (app.py): Invalid LOG_LEVEL '{log_level_app_str_final_cfg_v4}'. Using INFO.", file=sys.stderr); log_level_app_str_final_cfg_v4 = "INFO"
logging.basicConfig(level=getattr(logging, log_level_app_str_final_cfg_v4, logging.INFO), 
...
STREAMLIT_VERSION_GE_1_30_APP_V3 = False # Renamed var
STREAMLIT_PAGE_LINK_AVAILABLE_APP_V3 = False # Renamed var
try:
    from packaging import version 
    st_version_obj_v3 = version.parse(st.__version__) 
    if st_version_obj_v3 >= version.parse("1.30.0"): STREAMLIT_VERSION_GE_1_30_APP_V3 = True
    if hasattr(st, 'page_link'): STREAMLIT_PAGE_LINK_AVAILABLE_APP_V3 = True
    if not STREAMLIT_VERSION_GE_1_30_APP_V3: logger.warning(f"Streamlit version {st.__version__} < 1.30.0. Some UI features might use fallbacks.")
except Exception as e_st_ver_final_cfg_val_app_v3: logger.warning(f"Could not accurately determine Streamlit version/features: {e_st_ver_final_cfg_val_app_v3}")

if not importlib.util.find_spec("plotly"): logger.warning("Plotly not installed. Visualization features may fail.")

# --- Page Configuration ---
page_icon_path_obj_app_main_cfg_final_val_v3 = Path(settings.APP_LOGO_SMALL_PATH) 
final_page_icon_str_app_main_cfg_final_val_v3: str = str(page_icon_path_obj_app_main_cfg_final_val_v3) if page_icon_path_obj_app_main_cfg_final_val_v3.is_file() else "🌍"
if final_page_icon_str_app_main_cfg_final_val_v3 == "🌍": logger.warning(f"Page icon not found: '{page_icon_path_obj_app_main_cfg_final_val_v3}'. Using '🌍'.")
st.set_page_config(
    page_title=f"{settings.APP_NAME} - System Overview", page_icon=final_page_icon_str_app_main_cfg_final_val_v3,
...
except Exception as e_theme_main_app_cfg_final_val_app_v3: logger.error(f"Error applying Plotly theme: {e_theme_main_app_cfg_final_val_app_v3}", exc_info=True); st.error("Error applying visualization theme.")

@st.cache_resource
def load_global_css_styles_app_final_cfg_val_ui_app_v3(css_path_str_app_final_cfg_val_ui_app_v3: str): # Renamed function
    css_path_app_final_cfg_val_ui_app_v3 = Path(css_path_str_app_final_cfg_val_ui_app_v3)
    if css_path_app_final_cfg_val_ui_app_v3.is_file():
        try:
            with open(css_path_app_final_cfg_val_ui_app_v3, "r", encoding="utf-8") as f_css_app_final_cfg_val_ui_app_v3: st.markdown(f'<style>{f_css_app_final_cfg_val_ui_app_v3.read()}</style>', unsafe_allow_html=True)
            logger.debug(f"Global CSS loaded: {css_path_app_final_cfg_val_ui_app_v3}")
        except Exception as e_css_main_app_final_cfg_val_ui_app_v3: logger.error(f"Error applying CSS {css_path_app_final_cfg_val_ui_app_v3}: {e_css_main_app_final_cfg_val_ui_app_v3}", exc_info=True); st.error("Styles could not be loaded.")
    else: logger.warning(f"CSS file not found: {css_path_app_final_cfg_val_ui_app_v3}"); st.warning("Application stylesheet missing.")
if settings.STYLE_CSS_PATH_WEB: load_global_css_styles_app_final_cfg_val_ui_app_v3(settings.STYLE_CSS_PATH_WEB)

# --- Main Application Header ---
header_cols_app_ui_final_cfg_val_ui_val_app_v3 = st.columns([0.12, 0.88]) # Renamed var
with header_cols_app_ui_final_cfg_val_ui_val_app_v3[0]:
...
with header_cols_app_ui_final_cfg_val_ui_val_app_v3[1]: st.title(html.escape(settings.APP_NAME)); st.subheader("Transforming Data into Lifesaving Action at the Edge")
st.divider()
...
core_principles_main_app_v5_val_app_v3 = [
...
num_cols_core_principles_v5_val_app_v3 = min(len(core_principles_main_app_v5_val_app_v3), 2)
if num_cols_core_principles_v5_val_app_v3 > 0:
    cols_core_principles_ui_v5_val_app_v3 = st.columns(num_cols_core_principles_v5_val_app_v3)
    for idx_core_v5_val_app_v3, (title_core_v5_val_app_v3, desc_core_v5_val_app_v3) in enumerate(core_principles_main_app_v5_val_app_v3):
        with cols_core_principles_ui_v5_val_app_v3[idx_core_v5_val_app_v3 % num_cols_core_principles_v5_val_app_v3]:
            st.markdown(f"##### {html.escape(title_core_v5_val_app_v3)}"); st.markdown(f"<small>{html.escape(desc_core_v5_val_app_v3)}</small>", unsafe_allow_html=True)
    ...
    for nav_item_final_app_cfg_item_val_v4_app_v3 in role_navigation_config_app_final_cfg_list_val_app_v3:
        ...
        with nav_cols_ui_final_app_cfg_val_ui_v4_app_v3[current_col_idx_nav_final_cfg_val_ui_v4_app_v3 % num_nav_cols_final_app_cfg_val_ui_v4_app_v3]:
            ...
            with st.container(**container_args_final_app_cfg_val_v4_app_v3):
                ...
                st.markdown(f"<small>{nav_item_final_app_cfg_item_val_v4_app_v3['desc']}</small>", unsafe_allow_html=True) # Assuming desc is safe HTML
