...
try:
    ...
    logger_init.error(f"Critical import error in orchestrator.py: {e}. Ensure all model files exist in 'analytics' package.", exc_info=True)
    raise
...
class AIModelOrchestrator:
    ...
    Ensures a consistent and correct order of operations for data enrichment.
    """
    def __init__(self):
        ...
        # The sequence of models to apply. Order is critical.
    ...
    def apply_all_enrichment_models(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        logger.info(f"({self.module_log_prefix}) Starting AI model application on DataFrame of shape {df.shape}.")
        ...
        for model_name, model_func, output_col_name in self.models_to_apply:
            try:
                ...
                logger.info(f"({self.module_log_prefix}) Successfully applied {model_name}.")
        ...
        logger.info(f"({self.module_log_prefix}) AI model application completed.")
        return enriched_df, metadata

# --- Public Factory Function (Preserved for backward compatibility) ---

def apply_ai_models(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """
    Public-facing function to instantiate and run the AIModelOrchestrator.
