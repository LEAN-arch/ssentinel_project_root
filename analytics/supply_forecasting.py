...
def generate_linear_forecast(
    ...
    for _, row in latest_status.iterrows():
        ...
        item_df = pd.DataFrame({
            'item': row['item'], 'forecast_date': dates,
...
def generate_prophet_forecast(
    ...
    final_df.rename(columns={
        'ds': 'forecast_date', 'yhat': 'predicted_daily_consumption',
        'yhat_lower': 'consumption_lower_bound', 'yhat_upper': 'consumption_upper_bound',
    }, inplace=True)
    return final_df
