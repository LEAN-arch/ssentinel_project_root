...
# SME PLATINUM STANDARD - KPI ANALYSIS & TRENDING (V5 - DEFINITIVE FIX)
...
# SME FIX: Import directly from the submodule
...
def generate_kpi_analysis_table(full_df: pd.DataFrame, start_date: date, end_date: date) -> pd.DataFrame:
    ...
    for name, (kpi_key, trend_col, trend_agg, higher_is_better) in kpi_definitions.items():
        ...
        if pd.notna(current_val) and pd.notna(prev_val) and abs(prev_val) > 1e-9:
            ...
            change_str = f"{change:+.1f}%"; is_good_change = (change > 0) if higher_is_better else (change < 0)
