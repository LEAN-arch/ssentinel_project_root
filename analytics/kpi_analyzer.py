...
# SME PLATINUM STANDARD - KPI ANALYSIS & TRENDING (V4 - DEFINITIVE UNHASHABLEPARAM FIX)
...
# SME FIX: Import both the cached and non-cached trend functions
from data_processing.aggregation import get_cached_clinic_kpis, get_cached_trend, _calculate_trend
...
def generate_kpi_analysis_table(full_df: pd.DataFrame, start_date: date, end_date: date) -> pd.DataFrame:
    ...
    for name, (kpi_key, trend_col, trend_agg, higher_is_better) in kpi_definitions.items():
        ...
        # SME FIX: Use the correct trend function based on the type of agg_func
        if isinstance(trend_agg, str):
            # Use the fast, cached function for standard string aggregations
            trend_series = get_cached_trend(df=trend_df_subset, value_col=trend_col, date_col='encounter_date', freq='W', agg_func=trend_agg)
        else:
            # Use the non-cached function for the single custom callable aggregation
            trend_series = _calculate_trend(df=trend_df_subset, value_col=trend_col, date_col='encounter_date', freq='W', agg_func=trend_agg)
